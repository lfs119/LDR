...
def sps(smiles):
    """
    Computes RDKit's Spacial Score
    """
    if smiles is None:
        return None
    mol = Chem.MolFromSmiles(smiles)
    return SPS(mol, normalize=True)
...
def nsps(smiles):
    ...
    if smiles is None:
        return None
    mol = Chem.MolFromSmiles(smiles)
    return SPS(mol, normalize=False)

def qrci(smiles):
    if smiles is None:
        return None
    calculator = QRCICalculator(weights='mean')
    return calculator(smiles)
