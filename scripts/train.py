...
def train(rank, 
    ...
    #                d_model, d_ff, num_layers, num_heads, activation, dropout).to(device)   # FRATTVAE_Enhanced
    
    model = FRATTVAE_Enhanced(num_labels, max_depth, max_degree, feat_dim, latent_dim, 
                   d_model, d_ff, num_layers, num_heads, activation, dropout, prop_dim=0, use_gnn=False).to(device) 
    ...
    if load_epoch < 0:
        pretrained_model = '/raid/home/xukai/FRATTVAE/results/chembl_36_20251023_r1r10w1100w900_standardized_f_dyn_oral_struct_1031/models/model_best.pth'
        checkpoint = torch.load(pretrained_model)
        current_state_dict = model.state_dict()
        filtered_state_dict = {}
        for param_name, param_tensor in checkpoint.items():
            if param_name in current_state_dict:  
                if param_tensor.shape == current_state_dict[param_name].shape: 
                    filtered_state_dict[param_name] = param_tensor
                else:
                    
                    print(f"⚠️ 跳过 {param_name}：形状不匹配 | checkpoint 形状 {param_tensor.shape} ≠ 模型预期 {current_state_dict[param_name].shape}")
            else:
                print(f"⚠️ 跳过 {param_name}：当前模型无此层")

        # 5. 加载过滤后的参数（strict=False 允许剩余不匹配键，此处已过滤，也可设为 True）
        ...
        for param in model.fc_dec.parameters():
            param.requires_grad = True
        for param in model.fc_memory.parameters():
            param.requires_grad = True
        if rank==0: print(f'model loaded:{pretrained_model}')
