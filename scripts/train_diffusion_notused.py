...
def train(rank, 
    ...
    vae = PropWrapper(model, pmodel).to(device)
    vae.eval()

    sample_size = 16
    #unet model
    model = UNet2DModel(sample_size=sample_size, in_channels= 1, out_channels= 1, \
                      layers_per_block=8, block_out_channels= [256, 256, 512, 512], \
                        down_block_types=["DownBlock2D", "DownBlock2D", "AttnDownBlock2D", "DownBlock2D", ], up_block_types=["UpBlock2D", "AttnUpBlock2D", "UpBlock2D", "UpBlock2D", ])
    ...
    for epoch in range(load_epoch, epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            # forward
            with torch.no_grad():
                 z, mu, ln_var, output, pred_prop = vae(features, positions, 
                                                     src_mask, src_pad_mask, 
                                                     tgt_mask, tgt_pad_mask) 
            bs = z.shape[0]
            clean_images = z.reshape(bs, 1, sample_size, sample_size)         # output: shape= (B, L+1, num_labels)
            ...
            if rank==0:
                train_losses.append([loss.item()/n_gpu])
            ...
            if valid:
                ...
                with torch.no_grad():
                    for data in valid_loader:
                        ...
                        if rank == 0:
                            valid_losses.append([val_loss.item()/n_gpu])
                if rank==0: 
