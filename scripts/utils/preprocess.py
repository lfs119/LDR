...
try:
    ...
    print('[ERROR] biosynfoni is not installed. Please install it first.')
...
def frag2ecfp(frag, radius: int= 2, n_bits: int= 2048, useChiral: bool= True, ignore_dummy: bool= False, biosynfoni=False ):
    if ignore_dummy:
        # 虚拟原子替换为H 并移除
...
def frag2unimol_smile(frag, ignore_dummy: bool= False):
    if ignore_dummy:
        # 虚拟原子替换为H 并移除
        frag = Chem.RemoveHs(AllChem.ReplaceSubstructs(frag, Chem.MolFromSmiles('*'), Chem.MolFromSmiles('[H]'), replaceAll= True)[0])
...
def _smilesToMorganFingarPrintsAsBitVect(smiles, radius: int, n_bits: int, useChiral: bool= True, ignore_dummy: bool= True, unimol: bool= False, biosynfoni=False):
    if unimol:
        ecfp_bits = frag2unimol_smile(Chem.MolFromSmiles(smiles),ignore_dummy= ignore_dummy )
        ecfp = 1
    if biosynfoni:
        ecfp_bits = np.array(Biosynfoni(Chem.MolFromSmiles(smiles)).fingerprint, dtype= int).tolist() 
        ecfp = 1
    else:
        ecfp = frag2ecfp(Chem.MolFromSmiles(smiles), radius, n_bits, useChiral= useChiral, ignore_dummy= ignore_dummy)
        ecfp_bits = np.array(ecfp, dtype= int).tolist()    #2048

    return ecfp, ecfp_bits


def SmilesToMorganFingetPrints(fragments: list, n_bits: int, dupl_bits: int= 0, radius: int= 2, 
                               ignore_dummy: bool= True, useChiral: bool= True, n_jobs: int= 20, unimol: bool= False, biosynfoni: bool= False):
    """
    fragments: a list of fragment smiles.
    distinct_ecfp: If fragments list includes same ecfp, add bits to distinguish ecfp.
    """
    results = Parallel(n_jobs= n_jobs)(delayed(_smilesToMorganFingarPrintsAsBitVect)(f, radius, n_bits, useChiral= useChiral, ignore_dummy= ignore_dummy, unimol=unimol, biosynfoni=biosynfoni) for f in fragments)
    ecfps, ecfp_list = map(list, zip(*results))

    if dupl_bits > 0: # dupl_bits=0
