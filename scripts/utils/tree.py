...
class FragmentTree:
    ...
    def sinusoidal_encoding(self, x: torch.Tensor, d_model: int) -> torch.Tensor:
        """
        x: [N] integer tensor
        ...
        freqs = torch.exp(torch.arange(0, d_model, 2, dtype=torch.float32, device=device) * (-np.log(10000.0) / d_model))
        encoding = torch.zeros(*x.shape[:-1], d_model, device=device)  # [N, d_model]
        encoding[:, 0::2] = torch.sin(x * freqs)
        encoding[:, 1::2] = torch.cos(x * freqs)
        return encoding
    ...
    def set_all_positional_encoding(self, d_pos: int= None, n: int= None):
        ...
        # # Step 1: 计算 depth 和 degree
        # depths = torch.zeros(N, dtype=torch.long)
        # degrees = g.out_degrees()  # 在 DGL 中，out_degree 是子节点数（有向树）
        ...
        # roots = (in_degrees == 0).nonzero().squeeze(-1).tolist()
        # for r in roots:
        #     queue.append((r, 0))
        #     visited[r] = True
...
#################################################
        ...
        for nid in self.dgl_graph.nodes()[1:]:    
            ...
            # positional_encoding[children.index(nid)] = 1.0 + depths[nid] / self.max_depth + degrees[nid] / self.max_degree
        ...
        # pos_original = self.dgl_graph.ndata['pos']  # [N, d_pos]

        # # Depth Encoding
        # depth_dim = 16
        # depth_pos = self.sinusoidal_encoding(depths, depth_dim)  # [N, depth_dim]

        # # Degree Encoding
        # degree_dim = 16
        # degree_pos = self.sinusoidal_encoding(degrees, degree_dim)  # [N, degree_dim]

        # # 拼接所有 positional encoding
        # # pos = torch.cat([depth_pos, degree_pos, pos_original], dim=-1)  # [N, 16+16+d_pos]

        #    # 存入 DGL 图
        # g.ndata['pos'] = pos_original
        # g.ndata['depth'] = depths
        # g.ndata['degree'] = degrees

        # 可选：返回统计信息
        # print(f"[Tree Encoding] Max depth: {max_depth}, Max degree: {max_degree}, Final pos dim: {pos.size(1)}")

    def set_positional_encoding(self, nid: int, num_sibling: int= None, d_pos: int= None, add_pose = 0):
        d_pos = d_pos if d_pos else self.max_depth * self.max_degree + add_pose
...
def get_tree_features(frag_indices: list, ecfps: torch.Tensor, bond_types: list, bondMapNums: list, 
    ...
    src, dst = tree.dgl_graph.edges()
    edge_index = torch.stack([src, dst], dim=0).long()  # [2, E]

    return fids, features, positions, edge_index
